...
int dir_logica_a_fisica(t_cpu* cpu, t_pcb* pcb, int dir_logica, op_memoria_message instructions_code) {
	...
	int tabla_2do_nivel = get_tabla_segundo_nivel(cpu, tabla_1er_nivel, entrada_tabla_1er_nivel, instructions_code); // Preguntar a memoria, pasando tabla_1er_nivel y entrada_tabla_1er_nivel como datos.
	int marco = get_marco(cpu, tabla_2do_nivel, entrada_tabla_2do_nivel, instructions_code); // Preguntar a memoria, pasando tabla_2do_nivel y entrada_tabla_2do_nivel como datos.
	agregar_entry_tlb(cpu, numero_pagina, marco);

	return marco * mem_config->size_pagina + desplazamiento;
}

int get_tabla_segundo_nivel(t_cpu* cpu, int tabla_nivel, int entrada_tabla_nivel, op_memoria_message instructions_code){
	return get_tabla_marco_from_memoria(cpu, tabla_nivel, entrada_tabla_nivel, TABLA_SEGUNDO_NIVEL, instructions_code);
}

int get_marco(t_cpu* cpu, int tabla_nivel, int entrada_tabla_nivel, op_memoria_message instructions_code){
	return get_tabla_marco_from_memoria(cpu, tabla_nivel, entrada_tabla_nivel, MARCO, instructions_code);
}

int get_tabla_marco_from_memoria(t_cpu* cpu, int tabla_nivel, int entrada_tabla_nivel, int OPERACION, op_memoria_message ES_MODIFICADO){
	int tabla_segundo_nivel, op_code, marco_to_swap;
	t_cpu_paquete* paquete = malloc(sizeof(t_cpu_paquete));
	void* to_send = serialize_mmu_memoria(paquete, tabla_nivel, entrada_tabla_nivel, OPERACION, ES_MODIFICADO);
